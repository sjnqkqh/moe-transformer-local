...
@torch.no_grad()
def generate(model, tokenizer, prompt: str, max_new_tokens=100,
             temperature=0.8, top_k=50, top_p=0.9, repetition_penalty=1.2, device="cpu"):
    """
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, required=True, help="체크포인트 파일 (.pt) 경로")
    parser.add_argument("--tokenizer_dir", type=str, default="tokenizer/korean_output", help="토크나이저 디렉토리 경로")
    parser.add_argument("--prompt", type=str, required=True, help="생성할 시작 프롬프트 또는 유저 메시지")
    parser.add_argument("--chat", action="store_true", help="대화 인터페이스 사용 여부 (<user>... 형태로 포맷팅)")
    parser.add_argument("--max_new_tokens", type=int, default=100)
    parser.add_argument("--temperature", type=float, default=0.8)
    parser.add_argument("--top_k", type=int, default=50)
    parser.add_argument("--top_p", type=float, default=0.9)
    parser.add_argument("--repetition_penalty", type=float, default=1.2, help="반복 토큰 페널티")
    parser.add_argument("--device", type=str, default=None, help="실행 디바이스 (cuda, mps, cpu 등)")
