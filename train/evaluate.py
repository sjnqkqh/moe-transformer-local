...
def evaluate(args):
    ...
    if not ckpts:
        raise FileNotFoundError(
            f"'{args.checkpoint_pattern}' 패턴과 매치되는 체크포인트를 {args.ckpt_dir} 에서 찾을 수 없습니다."
        )
    ...
    if not os.path.exists(val_npy):
        raise FileNotFoundError(
            f"Validation dataset not found at {val_npy}. Please run prepare_data.py first."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file", type=str, default="drive_mock/reports/evaluation_report.json"
    )
