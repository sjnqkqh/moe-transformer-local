...
def train_tokenizer(
                            ...
                            for row in ds:
                                ...
                                if "text" in row:
                                    ...
                                    text = (
                                        row.get("question", "")
                                        + " "
                                        + row.get("answer", row.get("assistant", ""))
                                    )
                                elif "user" in row:
                                    text = (
                                        row.get("user", "")
    ...
    fast_tokenizer = PreTrainedTokenizerFast(
        ...
        additional_special_tokens=(
            ["<user>", "<assistant>", "<sep>"] if korean else None
        ),
    )
    ...
    assert (
        test_text == decoded
    ), "정밀 복구(Decode) 결과물과 원본이 완전히 일치하지 않습니다!"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="토크나이저 학습에 추가할 HuggingFace 데이터셋 이름 목록 (샘플링됨)",
    )
    parser.add_argument(
        "--hf_samples",
        type=int,
        default=10000,
        help="HF 데이터셋당 샘플링할 row 수 (기본값: 10000)",
    )
