...
class TestDropoutRegularization(unittest.TestCase):
    ...
    def test_dropout_layers_present(self):
        ...
        config = MoETransformerConfig(dropout=0.1)
        model = MoETransformer(config)
        
        # 1. MultiHeadAttention 드롭아웃 확인 (짝수/홀수 블록 모두)
        ...
        # 2. Dense FFN 및 MoE FFN 드롭아웃 확인
        # 홀수 레이어 (Dense)
        self.assertTrue(hasattr(model.layers[1].ffn.ffn, "dropout"))
        self.assertEqual(model.layers[1].ffn.ffn.dropout.p, 0.1)
        
        # 짝수 레이어 (MoE)
        self.assertTrue(hasattr(model.layers[0].ffn, "dropout"))
        self.assertEqual(model.layers[0].ffn.dropout.p, 0.1)
